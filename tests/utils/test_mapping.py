...
def fixture_empty_graph(tmpdir) -> Graph:
    graph = Graph(sql_db_filename=f'{tmpdir}/empty.sqlite3')
    return graph
...
def test_update_third_party_props(empty_graph: Graph, testdata_dir: str):
    ...
    empty_graph.import_ontology(f"{testdata_dir}/error-db.owl")
...
def test_generate_props_dict_whole_graph(test_graph_simple: Graph):
    ...
    graph_dict_true = {
        ...
        "edge_2_c":{"type":"CausalEdge","iri":"cg_store#edge_2_c","hasConfidence":1.0,"hasTimeLag":5,"hasCause":["node_2"],"comment":["edge_2c_comment"],"hasCreator":["master_creator"],"hasEffect":["node_3_c"]},
        "edge_3_c":{"type":"CausalEdge","iri":"cg_store#edge_3_c","hasConfidence":0.5,"hasTimeLag":10,"hasCause":["node_2"],"comment":["edge_3c_comment"],"hasCreator":["master_creator"],"hasEffect":["node_3_c"]}
    }
...
def test_generate_props_dict_whole_graph_third_party(test_graph_third: Graph):
    ...
    graph_dict_true = {
        "Mushroom_1":{"type":"CausalNode","iri":"cg_store#Mushroom_1","isCausing":["Mushroom_Edge"],"hasCreator":["Creator_1"]},
        "Mushroom_2":{"type":"CausalNode","iri":"cg_store#Mushroom_2","isAffectedBy":["Mushroom_Edge","Mushroom_2_9801_Edge"]},
        "9801":{"type":"CausalNode","iri":"cg_store#9801","apiUrl":"http://localhost:8080/db/Errors/9801","errorCode":9801,"message":"Fehlertext asdf asdf","hasCreator":["Creator_1"],"isCausing":["Error_Edge","Mushroom_2_9801_Edge"],"hasEnvironment":"environment_1"},
        "5800":{"type":"CausalNode","iri":"cg_store#5800","apiUrl":"http://localhost:8080/db/Errors/5800","errorCode":5800,"comment":["test comment"],"isAffectedBy":["Error_Edge"],"message":"asdf"},
...
def test_generate_props_dict_wrong_prop(test_graph_simple: Graph, test_graph_third: Graph):
    ...
    with pytest.raises(ValueError):
        prop_dict = test_graph_simple.map._Mapping__add_properties_to_dict(individual=individual, indi_type="CausalEdge")
...
def test_validate_properties(test_graph_simple: Graph, test_graph_third: Graph):
    assert test_graph_simple.map._Mapping__validate_property('hasConfidence') == True
    assert test_graph_simple.map._Mapping__validate_property('hasEnvironment') == False
    assert test_graph_third.map._Mapping__validate_property('hasConfidence') == True
    assert test_graph_third.map._Mapping__validate_property('hasEnvironment') == True
